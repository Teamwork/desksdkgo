...
// logError logs an error with structured fields if a logger is available
func (s *Service[T, L]) logError(msg string, attrs ...slog.Attr) {
	if s.client != nil && s.client.logger != nil {
		s.client.logger.LogAttrs(context.Background(), slog.LevelError, msg, attrs...)
	}
}
